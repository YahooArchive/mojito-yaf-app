YUI.add('mojito-yaf', function (Y, NAME) {
    ...
    MOJITO_NS.Builder = Y.Base.create('Builder', Y.Base, [],
        {
            initializer: function (params) {
                ...
                this.set('configData', instData);

                instKeys = Object.keys(instData);

                for (i = 0; i < instKeys.length; i++) {
                    this.registerControllerInstance(instKeys[i], params.app);
                }
            },

            registerControllerInstance: function (instanceName, appObj) {
                ...
                if (!(controllerTypeName =
                            this.get('configData')[instanceName])) {
                    return;
                }

                parts = controllerTypeName.split('.');
                ...
                //  type can find it
                controllerInst = new controllerType(
                                {id: instanceName, appInst: appObj});
    ...
    MOJITO_NS.Handler = Y.Base.create('Handler', Y.Base, [],
        {
            ...
            setupRoutes: function (mojitID, appInst) {
                ...
                appInst.route('/' + mojitID  + ':*action',
                              this.routeToController.bind(this));
    ...
    MOJITO_NS.Controller.findAllMojits = function () {
        var allMojitNodes,
            allMojits;
